function [s, U, V, f, time, R] = ERFDRiterinfo(r, s0, U0, V0, g0, g1, a, b, c, subclass, Delta, iter)
%% Description
...
tic
for i = 2:iter
    r_now = length(s{i-1}); % rank of the current iterate
    [s{i}, U{i}, V{i}, f(i)] = ERFDmap(m, n, r, r_now, s{i-1}, U{i-1}, V{i-1}, g0, g1, a, b, c, subclass);
    if r_now == r && s{i-1}(r) <= Delta % rank reduction mechanism
        R(1) = R(1)+1;
        [s_R, U_R, V_R, f_R] = ERFDmap(m, n, r, r-1, s{i-1}(1:r-1), U{i-1}(:, 1:r-1), V{i-1}(:, 1:r-1), g0, g1, a, b, c, subclass);
        if f_R < f(i)
            R(2) = R(2)+1;
            s{i} = s_R;
            U{i} = U_R;
